...
function counts(){
  ...
  members.forEach(m => {
    ...
    if(m.missa_dia){
      miss[m.missa_dia] = (miss[m.missa_dia] || 0) + 1;
    }
  ...
  document.querySelector("#app").innerHTML = `
    <header>
      ...
      <div>
        ...
        <p>Preceptorado Nossa Senhora da Penha · 2026</p>
      </div>
    </header>

    <nav>
      <button class="active" data-view="geral">
        Quadro Geral
      </button>
...
function show(view){
  ...
  if(view === "geral"){
    ...
    el.innerHTML = `
      ...
      <section class="intro">
        Este quadro reúne, sem identificação nominal, os registros da prática da Liturgia das Horas e da participação na Missa ferial no âmbito do Preceptorado Nossa Senhora da Penha, organizados segundo os dias da semana, em conformidade com a Regra da <i>Militia Sanctæ Mariæ</i>.
      </section>
      ...
      <div class="cards">
        ...
        ${HOURS.map(h => `
          <div class="card">
            <b>${h}</b>
            <strong>
              ${Object.values(c[h]).reduce((a,b)=>a+b,0)}
            </strong>
    ...
    el.innerHTML = `
      ...
      <p class="muted">
        Selecione seu nome para carregar seu registro atual.
      </p>
    ...
    <label>
      ...
      <select id="missa">
        ...
        ${[
          ...
          <option ${selected.missa_dia === x ? "selected" : ""}>
            ${x}
          </option>
        `).join("")}
      ...
      ${DAYS.map(d => `
        ...
        <label>
          ...
          <select id="${d[0]}">
            ...
            ${HOURS.map(h => `
              <option ${selected[d[0]] === h ? "selected" : ""}>
                ${h}
              </option>
            `).join("")}
  ...
  document
    ...
    .onclick = async () => {
      ...
      const updated = {
        ...selected,
        missa_dia:
          document.querySelector("#missa").value || null
      };
